...
def conditional(
        Xnew, X, kern, f, full_cov=False, q_sqrt=None, whiten=False, jitter_level=1e-6, return_Lm=False
):
    """
    ...
    Kmm = kern.K(X, X, X_inducing=True, X2_inducing=True)
    Lm = torch.linalg.cholesky(Kmm + torch.eye(num_data, dtype=X.dtype, device=X.device) * jitter_level)
